...
//Creating a function to select the Query's
...
function setInitialValues() {
    isJumping = false //Making sure the Charcter isn't jumping
...
function initRandomHoles() {
    hole.addEventListener( 'animationiteration', _ => {
        const fromHeight = 57 * window.innerHeight / 100 //Height adjustments for Mobile
        const toHeight = 97 * window.innerHeight / 100

        const randomTop = getRandomNumber( fromHeight, toHeight ) //getRandomNumber won't work because we dont have this Function
                                                                //we create this function in Utils folder
...
// Function to Slow Down the Pipes Speed when Windows is Enlarged
...
//A function for when the game first LOAD's (Sort of like the MAIN in java)
function gameInit() {
    getElements()
    setInitialValues()
    initRandomHoles()
...
gameInit() //Calling the Function: w/o calling this the Pipes won't Move.